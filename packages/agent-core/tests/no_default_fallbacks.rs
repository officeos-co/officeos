...
async fn test_runtime_config_rejects_zero_gateway_port() {
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(ResponseTemplate::new(200).set_body_json(helpers::canned_payload_zero_port()))
        .expect(1)
        .mount(&server)
        .await;
...
async fn test_bootstrap_rejects_empty_system_prompt() {
    ...
    Mock::given(method("GET"))
        .and(path(format!("/api/agents/{CANNED_AGENT_ID}")))
        .respond_with(
            ResponseTemplate::new(200).set_body_json(helpers::canned_payload_empty_prompt()),
        )
        .expect(1)
        .mount(&server)
        .await;
