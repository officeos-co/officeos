...
async fn count_reflects_cache() {
    ...
    mem.store("a", "A", MemoryCategory::Core, None)
        .await
        .unwrap();
    mem.store("b", "B", MemoryCategory::Core, None)
        .await
        .unwrap();
...
async fn list_filters_by_category() {
    let mem = ObsidianMemory::new();
    mem.store("a", "Core A", MemoryCategory::Core, None)
        .await
        .unwrap();
    mem.store("b", "Daily B", MemoryCategory::Daily, None)
        .await
        .unwrap();
