...
/// A parsed skill from the GraphQL schema.
#[derive(Debug, Clone)]
...
///
/// Convention: field names like `notionSearch`, `githubRepos` are split
/// into skill="notion" action="search", skill="github" action="repos".
/// Fields without a recognizable prefix (like `ping`) are ignored.
fn parse_schema_to_skills(fields: &[FieldInfo]) -> HashMap<String, SkillInfo> {
    ...
    for field in fields {
        ...
        for prefix in &prefixes {
            let camel_prefix = *prefix;
            if field.name.starts_with(camel_prefix) && field.name.len() > camel_prefix.len() {
                let action_part = &field.name[camel_prefix.len()..];
                // Convert PascalCase remainder to snake_case CLI name
                let cli_action = pascal_to_snake(action_part);

                let args: Vec<ActionArg> = field
                    .args
                    .iter()
                    .filter(|a| a.name != "ct") // skip CancellationToken
                    .map(|a| {
                        let (type_name, required) = resolve_type(&a.arg_type);
                        ActionArg {
                            name: camel_to_snake(&a.name),
                            type_name,
                            required,
        ...
        if !matched {
            // Skip non-skill fields like "ping"
        }
    }
...
fn resolve_type(t: &TypeRef) -> (String, bool) {
    match t.kind.as_deref() {
        Some("NON_NULL") => {
            let inner = t.of_type.as_ref().map(|o| type_name(o)).unwrap_or("Any".to_string());
            (inner, true)
        }
        _ => (type_name(t), false),
    }
...
fn extract_return_fields(t: &TypeRef) -> Vec<String> {
    // Unwrap NON_NULL and LIST wrappers to find the concrete type
