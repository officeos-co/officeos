...
fn cost_optimized_selects_cheapest_provider() {
    ...
    let (idx, model) = router.resolve_cost_optimized("hint:cost-optimized", &prices, false, false);
...
async fn stream_chat_with_history_routes_hint_to_correct_provider_and_model() {
    ...
    let mut stream =
        router.stream_chat_with_history(&messages, "hint:reasoning", 0.0, StreamOptions::new(true));
