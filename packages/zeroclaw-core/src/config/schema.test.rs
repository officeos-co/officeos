...
async fn config_default_has_sane_values() {
    ...
    // Provider/model are no longer compiled-in defaults — they must
    // be supplied by the caller and `Agent::from_config` enforces it.
...
async fn parse_extra_headers_env_with_url_value() {
    let headers = parse_extra_headers_env("HTTP-Referer:https://github.com/zeroclaw-labs/zeroclaw");
...
#[test]
async fn env_override_zero_claw_provider_overrides_configured_provider() {
    let _env_guard = env_override_lock().await;
