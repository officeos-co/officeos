...
public interface IOrgBillingService
{
    ...
    Task<(long Remaining, bool OverBudget)> CheckCreditBudgetAsync(string orgId, CancellationToken ct = default);
    Task<string> CreateCustomerAsync(string orgId, string email, CancellationToken ct = default);
    Task<string> CreateSubscriptionAsync(string customerId, string plan, CancellationToken ct = default);
