...
export default function EditAgentPage() {
  ...
  const agentId = Array.isArray(agentIdParam)
    ? agentIdParam[0]
    : agentIdParam;
  // Backend hooks still use "gatewayId" — treat agentId as the same value.
  const gatewayId = agentId;
  ...
  return (
    <DashboardPageLayout
      signedOut={{
        message: "Sign in to edit an agent.",
        forceRedirectUrl: `/agents/${agentId}/edit`,
      }}
      title={
        resolvedName.trim()
          ? `Edit agent — ${resolvedName.trim()}`
          : "Edit agent"
      }
      description="Update connection settings for this agent."
      isAdmin={isAdmin}
      adminOnlyMessage="Only organization owners and admins can edit agents."
    >
      <GatewayForm
        name={resolvedName}
        provider="openrouter"
