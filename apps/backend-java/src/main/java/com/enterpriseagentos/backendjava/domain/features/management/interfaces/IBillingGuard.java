...
public interface IBillingGuard {
    CompletableFuture<Boolean> isQuotaExceededAsync(UUID agentId);
    CompletableFuture<Void> throwIfQuotaExceededAsync(UUID agentId);
    CompletableFuture<Void> refreshCacheAsync(UUID agentId);
}
