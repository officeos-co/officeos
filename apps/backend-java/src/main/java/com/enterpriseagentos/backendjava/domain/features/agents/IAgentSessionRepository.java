...
public interface IAgentSessionRepository {
    CompletableFuture<AgentSessionRecord> getActiveAsync(UUID agentId);
    CompletableFuture<AgentSessionRecord> getByIdAsync(UUID sessionId);
    CompletableFuture<List<AgentSessionRecord>> listByAgentAsync(UUID agentId, int limit);
    CompletableFuture<AgentSessionRecord> createAsync(AgentSessionRecord session);
