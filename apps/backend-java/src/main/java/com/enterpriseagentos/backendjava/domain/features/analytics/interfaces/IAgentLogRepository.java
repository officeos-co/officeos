package com.enterpriseagentos.backendjava.domain.features.analytics.interfaces;
...
public interface IAgentLogRepository {
    CompletableFuture<List<AgentLogModel>> listAsync(UUID agentId, Instant before, int limit);
    CompletableFuture<List<AgentLogModel>> listAfterAsync(UUID agentId, UUID afterLogId, int limit);
    CompletableFuture<AgentLogModel> appendAsync(AgentLogModel model);
    CompletableFuture<Void> appendPairAsync(AgentLogModel toolCall, AgentLogModel toolResult);
    CompletableFuture<AgentLogModel> getByIdAsync(UUID id);
    CompletableFuture<Map<String, AgentLogModel>> getResultsByCorrelationAsync(UUID agentId, Collection<String> correlationIds);
    CompletableFuture<Void> deleteByAgentIdsAsync(List<UUID> agentIds);
    CompletableFuture<List<AgentLogModel>> listByAgentIdsAsync(List<UUID> agentIds, List<AgentLogType> types);
}
