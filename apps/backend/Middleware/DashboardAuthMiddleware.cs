...
public sealed class DashboardAuthMiddleware
{
    ...
    public async Task InvokeAsync(IMiddlewareContext context)
    {
        ...
        var user = http?.Items["User"] as EnterpriseAgentOs.Domain.Models.UserRecord;
        if (user is null)
        {
...
{
    public static EnterpriseAgentOs.Domain.Models.UserRecord GetUser(this IResolverContext context)
    {
        var http = context.Service<IHttpContextAccessor>().HttpContext;
        var user = http?.Items["User"] as EnterpriseAgentOs.Domain.Models.UserRecord
