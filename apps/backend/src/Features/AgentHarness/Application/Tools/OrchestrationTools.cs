                ...
                github_triggers = new
                {
                    ...
                    items = new
                    {
                        ...
                        properties = new
                        {
                            ...
                            auth_ref = new { type = "string", description = "Credential resource name for polling mode" },
...
internal sealed class AgentSpawnTool : IAgentTool
{
    ...
    public AgentSpawnTool(IPublisher publisher, Guid agentId, Guid? definitionId)
    {
    ...
    public ToolSchema Schema => new("agent_spawn",
        "Queue independent follow-up work for this agent session. Use for subagent-style decomposition without blocking the current message.",
        new
        {
            type = "object",
            properties = new
    ...
    public async Task<AgentResult<ToolResult>> ExecuteAsync(JsonElement args, CancellationToken ct = default)
    {
        ...
        var name = args.TryGetProperty("name", out var nameElement) ? nameElement.GetString() : null;
        ...
        var payload = JsonSerializer.Serialize(new
        {
            ...
            status = AgentWorkStatusKinds.Queued,
            mode,
            note = "Message queued for detached execution in the current agent session."
        }, new JsonSerializerOptions { WriteIndented = true });
