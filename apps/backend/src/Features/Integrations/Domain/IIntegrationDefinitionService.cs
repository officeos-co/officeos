...
public interface IIntegrationDefinitionService
{
    ...
    Task<IReadOnlyList<IntegrationDefinitionRecord>> ListCatalogAsync(Guid ownerId, Guid? workspaceId = null, CancellationToken ct = default);
    ...
    Task SaveOAuthCredentialAsync(Guid ownerId, Guid workspaceId, string provider, Dictionary<string, string> fields, IReadOnlyList<string> scopes, string? email, DateTime? expiresAtUtc, CancellationToken ct = default);
    Task ArchiveCredentialAsync(Guid ownerId, Guid workspaceId, string integrationName, CancellationToken ct = default);
