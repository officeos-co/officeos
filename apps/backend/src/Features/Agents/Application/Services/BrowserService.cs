...
{
    private readonly IBrowserSessionRepository _sessions;
    ...
    public async Task<BrowserSessionState> GetOrCreateAsync(Guid agentId, CancellationToken ct = default)
    {
        ...
        if (existing is not null)
        {
            ...
            {
                await _sessions.UpsertAsync(agentId, existing.RuntimeSessionId, existing.CookiesJson, ct);
                return state with { LastAccessedAt = DateTime.UtcNow };
            }
        }

        var profileName = AgentProfileName(agentId);
        var created = await _runtime.CreateSessionAsync(agentId, $"agent-{agentId:N}", profileName, ct);
        ...
        await _sessions.UpsertAsync(agentId, created.RuntimeSessionId, null, ct);
        return created;
    }
    ...
    public async Task StopAsync(Guid agentId, CancellationToken ct = default)
    {
        ...
        try
        {
            await _runtime.CloseSessionAsync(existing.RuntimeSessionId, ct);
        }
        catch (Exception ex)
        {
        ...
        await _sessions.DeleteByAgentAsync(agentId, ct);
    }
