...
public sealed class OrgBillingService : IOrgBillingService
{
    ...
    public OrgBillingService(EnterpriseAgentOs.Infrastructure.Configuration.StripeConfig config, EnterpriseAgentOs.Infrastructure.Persistence.EaosDbContext db, ILogger<OrgBillingService> logger)
    {
        _config = config;
        _db = db;
        _logger = logger;
