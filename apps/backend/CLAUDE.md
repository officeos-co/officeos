...
- **Api** (`EnterpriseAgentOs.Api`) — Entry point (`Program.cs`). GraphQL via HotChocolate:
...
- **Feature-sliced** — each feature (Agents, Mcp, Channels, Billing, Auth, etc.) has its own folder in each layer.
