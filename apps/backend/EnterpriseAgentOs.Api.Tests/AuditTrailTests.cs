...
public sealed class AuditTrailTests : IClassFixture<CustomWebApplicationFactory>
{
    private readonly CustomWebApplicationFactory _factory;

    public AuditTrailTests(CustomWebApplicationFactory factory) => _factory = factory;
    ...
    public async Task AuditLog_AgentIsolation()
    {
        ...
        var agent1Id = await TestHelpers.CreateAgentAsync(dashClient, "isolation-agent-1");
        var agent2Id = await TestHelpers.CreateAgentAsync(dashClient, "isolation-agent-2");

        await InstallAndConfigureNotionAsync(dashClient);

        // Execute skill only for agent 1
        var agent1Client = new MockAgentClient(_factory.CreateClient(), agent1Id);
