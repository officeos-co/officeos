...
def _render_skill_gateway_config_toml(*, backend_url: str, backend_token: str) -> str:
    """Build a minimal `config.toml` for an agent pod.

    Contains only the `[skills]` section the Skill Gateway runtime
    cares about. zeroclaw-core's config loader fills in defaults for
    every other field, so a partial TOML is safe.

    This file lands inside the per-agent vault ConfigMap (and the
    corresponding CouchDB vault database) alongside SOUL.md /
    IDENTITY.md / etc., and is read from the workspace directory at
    agent boot.
    """

    # Escape embedded quotes/backslashes defensively. The token is
    # base64-urlsafe + the backend URL is a config value, but we keep
    # this strict so a future URL change never accidentally breaks the
    # TOML parser.
    def _q(s: str) -> str:
        return s.replace("\\", "\\\\").replace('"', '\\"')

    return (
        "# Generated by the dashboard backend at agent provisioning time.\n"
        "# DO NOT EDIT BY HAND — regenerated on every agent create/update.\n"
        "\n"
        "[skills]\n"
        f'backend_url = "{_q(backend_url)}"\n'
        f'backend_token = "{_q(backend_token)}"\n'
        "backend_refresh_seconds = 30\n"
    )
...
class AgentLifecycleService(OpenClawDBService):
    ...
    async def _seed_agent_vault(
        ...
        *,
        ...
        # Inject a minimal `config.toml` containing only the [skills]
        # section pointing at this backend. zeroclaw-core reads it from
        # the workspace dir at boot and the Rust runtime uses it to
        # refresh capabilities from /api/agents/me/capabilities every
        # turn. `auth_token` is the raw agent token minted by
        # `persist_new_agent`; the agent uses it to authenticate its
        # backend calls.
        rendered["config.toml"] = _render_skill_gateway_config_toml(
            backend_url=settings.base_url,
            backend_token=auth_token,
        )
