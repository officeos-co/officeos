...
from app.api.gateway_ui_proxy import router as gateway_ui_proxy_router
...
api_v1.include_router(zeroclaw_router)
...
# Per-provider LLM API keys (Settings → Providers).
api_v1.include_router(providers_router)
# Reverse-proxy to each agent pod's embedded web dashboard.
api_v1.include_router(gateway_ui_proxy_router)
