...
export function useChannels(): {
  ...
  // matching type, override `name`/`description`/`protocol` with live values.
  const channels: Channel[] = mockChannels.map((m) => {
    const live = types.find((t) => t.slug === m.slug)
    return {
      ...m,
      name: live?.name ?? m.name,
      description: live?.description ?? m.description,
      protocol: live?.protocol ?? m.protocol,
  ...
  for (const t of types) {
    if (!mockChannels.find((m) => m.slug === t.slug)) {
      channels.push({
        name: t.name,
        slug: t.slug,
        logo: "",
        ...
        added: connectedSlugs.has(t.slug),
        onboarding: [],
        skillMd: "",
      })
    }
