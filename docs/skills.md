...
3. **Backend gateway** (`apps/backend/Entities/Skills/`) — Proxies execution requests, manages credential encryption/storage, and serves the dashboard API. The backend has zero hardcoded skill knowledge — everything comes from the runtime's `/manifests` endpoint.
